...
interface Subcategory {
  _id: string; name: string; slug: string; isActive: boolean; order: number;
  categoryId: Category | string; description?: string; createdAt: string;
}

const EMPTY = { name: "", slug: "", categoryId: "", description: "", isActive: true, order: 0 };
...
export default function SubcategoriesPage() {
  ...
  function openCreate() {
    setForm({ ...EMPTY, categoryId: cats[0]?._id ?? "" });
    setModal("create");
  }
  function openEdit(s: Subcategory) {
    setSelected(s);
    const catId = typeof s.categoryId === "object" ? s.categoryId._id : s.categoryId;
    setForm({ name: s.name, slug: s.slug, categoryId: catId, description: s.description ?? "", isActive: s.isActive, order: s.order });
    setModal("edit");
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const url    = modal === "create" ? "/api/admin/subcategories" : `/api/admin/subcategories/${selected!._id}`;
    const method = modal === "create" ? "POST" : "PATCH";
    const res    = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    const data   = await res.json();
    if (!res.ok) { toast("error", data.error ?? "Failed"); setSaving(false); return; }
    toast("success", modal === "create" ? "Subcategory created" : "Subcategory updated");
    setModal(null); load(); setSaving(false);
  }

  async function handleDelete() {
    setDeleting(true);
    const res  = await fetch(`/api/admin/subcategories/${delId}`, { method: "DELETE" });
    const data = await res.json();
    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-black text-white">Subcategories</h1>
            <p className="text-sm text-zinc-500">{subs.length} total</p>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#111]">
          {loading ? <Spinner /> : subs.length === 0 ? <Empty message="No subcategories yet" /> : (
                ...
                <tbody>
                  {subs.map(s => (
                    <tr key={s._id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3 text-zinc-400">{getCatName(s.categoryId)}</td>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === "create" ? "Add Subcategory" : "Edit Subcategory"}>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Name" required>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className={inputCls} placeholder="e.g. Basketball Uniforms" />
          </Field>
          <Field label="Slug">
      ...
      <ConfirmModal open={!!delId} title="Delete Subcategory"
        message="This will fail if products depend on it."
